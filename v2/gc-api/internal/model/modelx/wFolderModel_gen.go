...
func (m *defaultWFolderModel) Delete(ctx context.Context, id int64) error {
	...
	rudyGcWFolderParentIdNameSourceTypeKey := fmt.Sprintf("%s%v:%v:%v", cacheRudyGcWFolderParentIdNameSourceTypePrefix, data.ParentId, data.Name, data.SourceType)
	rudyGcWFolderPathSourceTypeKey := fmt.Sprintf("%s%v:%v", cacheRudyGcWFolderPathSourceTypePrefix, data.Path, data.SourceType)
...
func (m *defaultWFolderModel) FindOneByParentIdNameSourceType(ctx context.Context, parentId int64, name string, sourceType int64) (*WFolder, error) {
	rudyGcWFolderParentIdNameSourceTypeKey := fmt.Sprintf("%s%v:%v:%v", cacheRudyGcWFolderParentIdNameSourceTypePrefix, parentId, name, sourceType)
	var resp WFolder
...
func (m *defaultWFolderModel) FindOneByPathSourceType(ctx context.Context, path string, sourceType int64) (*WFolder, error) {
	rudyGcWFolderPathSourceTypeKey := fmt.Sprintf("%s%v:%v", cacheRudyGcWFolderPathSourceTypePrefix, path, sourceType)
	var resp WFolder
...
func (m *defaultWFolderModel) Insert(ctx context.Context, data *WFolder) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.ParentId, data.Name, data.Depth, data.Path, data.PathHash, data.CreatedOn, data.UpdatedOn, data.SourceType)
	}, rudyGcWFolderIdKey, rudyGcWFolderParentIdNameSourceTypeKey, rudyGcWFolderPathSourceTypeKey)
	return ret, err
}
...
func (m *defaultWFolderModel) Update(ctx context.Context, newData *WFolder) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.ParentId, newData.Name, newData.Depth, newData.Path, newData.PathHash, newData.CreatedOn, newData.UpdatedOn, newData.SourceType, newData.Id)
	}, rudyGcWFolderIdKey, rudyGcWFolderParentIdNameSourceTypeKey, rudyGcWFolderPathSourceTypeKey)
	return err
}
