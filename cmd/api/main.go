...
func main() {
	...
	// 创建通道
	invCh := make(chan *spider.Notification, 4)
	detailCh := make(chan *spider.Notification, 4)

	// 准备依赖

	conn := sqlx.NewMysql(c.DataSource)
	deps := svc.NewDeps(conn)
	...
	// invCh <- &spider.Notification{Info: spider.NotifyCrawActiveQueries}
