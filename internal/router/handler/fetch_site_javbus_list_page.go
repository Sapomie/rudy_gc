...
type fetchSiteJavbusSortQuery struct {
	...
	ByFilmBirthTime  fetchSiteJavbusSortLink
...
func (h *CrawlerPages) FetchSiteJavbusListPageMain(c *gin.Context) {
	...
	if ts, ok, err := parseOptionalDateStart(q.FilmBirthFrom); err != nil {
		c.String(http.StatusBadRequest, "JavBus 下载时间开始日期错误: %v", err)
		return
	} else if ok {
	...
	if ts, ok, err := parseOptionalDateEnd(q.FilmBirthTo); err != nil {
		c.String(http.StatusBadRequest, "JavBus 下载时间结束日期错误: %v", err)
		return
	} else if ok {
	...
	if ts, ok, err := parseOptionalDateStart(q.MediaBirthFrom); err != nil {
		c.String(http.StatusBadRequest, "JavBus M下载时间开始日期错误: %v", err)
		return
	} else if ok {
	...
	if ts, ok, err := parseOptionalDateEnd(q.MediaBirthTo); err != nil {
		c.String(http.StatusBadRequest, "JavBus M下载时间结束日期错误: %v", err)
		return
	} else if ok {
