...
func (l *CrawlLogic) CrawlDailyBestProcession(ctx context.Context, isSync bool, autoFetchSite bool) (err error) {
	...
	err = l.runParallel(ctx, postSteps...)
	return err
}
...
func (l *CrawlLogic) RunPostProcess(ctx context.Context, withCover bool, withTranslate bool, translateStatuses []int64) error {
	postSteps := make([]func(context.Context) error, 0, 2)
	if withCover {
		postSteps = append(postSteps, l.DownLoadAllPicture)
	}
	if withTranslate {
		postSteps = append(postSteps, func(ctx context.Context) error {
			return l.TranslateTitleWithStatuses(ctx, translateStatuses)
		})
	}
