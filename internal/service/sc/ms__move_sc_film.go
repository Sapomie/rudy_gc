...
func (l *ScService) MoveScFilm(ctx context.Context, scName string) error {
	gLists, err := l.glFindByFilters(ctx, scName, nil, 1, 100)
	if err != nil {
		return err
	}
	if len(gLists) == 100 {
	...
	dir, err := l.directoryFindOneByName(ctx, "v3")
	if err != nil {
		return err
	}
	...
	for _, m := range gLists {
		f, err := l.filmFindOneByMovieJavID(ctx, m.MovieJavId)
		if err != nil {
			return err
		}
		if f == nil {
