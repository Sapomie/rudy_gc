...
func (l *FetchLoopService) publishTaskProgress(jobID, taskType string, progress taskctx.Progress) {
	event := JobEvent{
		...
		PendingItems:      cloneManagedQueueItems(progress.PendingItems),
		RunningItems:      cloneManagedQueueItems(progress.RunningItems),
		DoneItems:         cloneManagedQueueItems(progress.DoneItems),
