...
func (s *FilmService) scanRoots(ctx context.Context, fctx *filmContext) (*ProcessFilmResponse, error) {
	...
	for _, p := range s.deps.Config.Film.Pairs {
		taskctx.ReportProgress(ctx, taskctx.Progress{
			Stage:   "film_root_begin",
			Message: fmt.Sprintf("开始扫描目录：%s", filepath.Clean(p.RootDir)),
		})
...
func (s *FilmService) walkOneRoot(
	...
	return filepath.WalkDir(root, func(p string, d fs.DirEntry, walkErr error) error {
		...
		taskctx.ReportProgress(ctx, taskctx.Progress{
			...
			Message:      fmt.Sprintf("已处理影片：%s", filepath.Base(p)),
			HandledCount: int(fctx.Processed),
			SuccessCount: int(fctx.Processed),
		})
		return nil
	})
}
...
func (s *FilmService) handleOneVideo(ctx context.Context, root, fullPath string, fctx *filmContext, info os.FileInfo) (*types.Film, error) {
	...
	// Upsert
	_, status, err := s.filmUpsert(ctx, film)
	if err != nil {
		return nil, err
	}
...
func (s *FilmService) removeMissingFilmFiles(ctx context.Context, fctx *filmContext) error {
	...
	for _, fi := range fctx.FilmExistMap {
		...
		film, err := s.filmFindOne(ctx, fi.Film.Id)
		if err != nil {
