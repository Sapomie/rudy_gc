...
type Folder struct {
	...
	ParentID  int64  `gorm:"column:parent_id;not null;default:0;index:idx_w_folder_parent_id;uniqueIndex:uk_w_folder_parent_name"` // 根=0
	Name      string `gorm:"column:name;type:varchar(191);not null;uniqueIndex:uk_w_folder_parent_name"`
	Depth     int8   `gorm:"column:depth;type:tinyint;not null;index:idx_w_folder_depth"`
	Path      string `gorm:"column:path;type:varchar(512);not null;uniqueIndex:uk_w_folder_path"`    // 512 保证可建唯一索引
