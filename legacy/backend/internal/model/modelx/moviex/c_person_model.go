...
type (
	CPersonModel interface {
		...
		FindOneByDisplayName(ctx context.Context, name string) (*CPerson, error)
...
func (m *customCPersonModel) FindOneByDisplayName(ctx context.Context, name string) (*CPerson, error) {
	name = strings.TrimSpace(name)
	if name == "" {
		return nil, ErrNotFound
	}

	query := "select " + cPersonRows + " from " + m.table + " where `name` = ? or `chinese` = ? or `alias` like ? escape '\\\\' order by case when `chinese` = ? then 0 when `name` = ? then 1 else 2 end, `movie_number` desc, `owned_w_media_number` desc, `id` asc limit 64"
	likePattern := "%" + escapeSQLLikeValue(name) + "%"

	var rows []*CPerson
	if err := m.QueryRowsNoCacheCtx(ctx, &rows, query, name, name, likePattern, name, name); err != nil {
		if errors.Is(err, sqlx.ErrNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
	...
	for _, row := range rows {
		...
		if strings.TrimSpace(row.Chinese) == name {
			return row, nil
		}
		if normalizePersonAliasToken(row.Name) == norm {
			return row, nil
...
SELECT DISTINCT
	...
	p.last_sc_event_time AS last_sc_event_time,
