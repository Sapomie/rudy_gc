...
type TorrentAlbumsPageQuery struct {
	AlbumName          string
	Page               int64
	PageSize           int64
	Sort               string
...
type TorrentAlbumsPageResult struct {
	SelectedAlbumID   int64
	SelectedAlbumName string
	Albums            []*TorrentAlbumOption
	Items             []*TorrentAlbumPageRow
	Total             int64
	Page              int64
	PageSize          int64
}

type TorrentAlbumOption struct {
	ID   int64
	Name string
}

type TorrentAlbumPageRow struct {
	ID             int64
	AlbumID        int64
	MovieJavID     string
	MovieName      string
...
func (s *Service) BuildTorrentAlbumsPage(ctx context.Context, query TorrentAlbumsPageQuery) (*TorrentAlbumsPageResult, error) {
	page := query.Page
	if page <= 0 {
		page = 1
	}
	...
	options := buildTorrentAlbumOptions(albums, selectedAlbumName)
	result := &TorrentAlbumsPageResult{
	...
	rows, err := s.deps.AlbumItemModel.ListPageRows(ctx, selectedAlbumID, offset, pageSize, torrentAlbumPageOrderBy(query.Sort, query.Order), filter)
	if err != nil {
		return nil, err
	}

	items := make([]*TorrentAlbumPageRow, 0, len(rows))
	for _, row := range rows {
		if row == nil {
			continue
		}
		items = append(items, &TorrentAlbumPageRow{
			ID:             row.Id,
			AlbumID:        row.AlbumId,
			...
			SourceTypeText: torrentAlbumSourceTypeText(row.SourceType),
...
func buildTorrentAlbumOptions(rows []*moviex.TAlbum, selectedAlbumName string) []*TorrentAlbumOption {
	options := make([]*TorrentAlbumOption, 0, len(rows)+1)
	hasSelected := false
	for _, row := range rows {
		if row == nil {
			continue
		...
		options = append(options, &TorrentAlbumOption{
			ID:   row.Id,
			Name: name,
		})
	}
...
func torrentAlbumSourceTypeText(sourceType string) string {
...
func torrentAlbumPageOrderBy(sortField string, sortOrder string) string {
	column := mapTorrentAlbumSortColumn(sortField)
	order := normalizeTorrentAlbumsSortOrder(sortOrder)
	return column + " " + order + ", `id` DESC"
}

func mapTorrentAlbumSortColumn(sortField string) string {
...
func normalizeTorrentAlbumsSortOrder(sortOrder string) string {
