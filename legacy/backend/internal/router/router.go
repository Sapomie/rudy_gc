...
func New(d *dep.Dep) *gin.Engine {
	...
	r.GET("/crawl-records", movieHTML.CrawlRecordListPage)
	...
	r.GET("/cast/:id", movieHTML.CastDetailPage)
	r.GET("/medias", movieHTML.MediaListPage)
	r.GET("/w-agg-events", movieHTML.WAggEventListPage)
	r.GET("/e-items", movieHTML.EItemListPage)
	r.GET("/album-items", func(c *gin.Context) {
		target := "/torrent-albums"
		if raw := c.Request.URL.RawQuery; raw != "" {
			target += "?" + raw
		}
		c.Redirect(http.StatusFound, target)
	})
	r.GET("/torrent-albums", movieHTML.TorrentAlbumsPage)
	r.GET("/movie-albums", movieHTML.MovieAlbumItemsPage)
	r.GET("/d-seeds", crawlerPages.DSeedListPage)
	...
	r.GET("/triggers/agg", crawlerPages.AggTriggersPage)
	...
	api := r.Group("/api")
	{
		...
		dSeedAPI := handler.NewDSeedAPI(d)
		eItemAPI := handler.NewEItemAPI(d)
		personAPI := handler.NewPersonAPI(d)
		scEventAPI := handler.NewScEventAPI(d)
		...
		api.POST("/movie/:movie/movie-album-item", movieAPI.AddMovieToMovieAlbum)
		api.POST("/movie-albums", movieAPI.CreateMovieAlbum)
		api.POST("/movie-albums/:albumID/items/remove", movieAPI.RemoveMovieAlbumItem)
		api.POST("/movie-albums/:albumID/execute-remove", movieAPI.ExecuteMovieAlbumRemove)
		api.POST("/movie-albums/:albumID/remove-downloaded-items-preview", movieAPI.PreviewRemoveDownloadedMovieAlbumItems)
		api.POST("/movie-albums/:albumID/remove-downloaded-items", movieAPI.RemoveDownloadedMovieAlbumItems)
		api.POST("/d-seeds", dSeedAPI.Create)
		api.POST("/d-seeds/:id", dSeedAPI.Update)
		api.POST("/sc-events/:name", scEventAPI.Update)
		...
		aggTriggerAPI := handler.NewAggTriggerAPI(d)
		...
		api.POST("/agg/w-media/backfill", aggTriggerAPI.BackfillWMediaAgg)
		api.POST("/agg/movie-release/backfill", aggTriggerAPI.BackfillMovieReleaseAgg)
