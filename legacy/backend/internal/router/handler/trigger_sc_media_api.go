...
func (h *ScMediaTriggerAPI) Plan(c *gin.Context) {
	mode := normalizeScMediaMoveReqMode(c.Query("mode"))
	scName := strings.TrimSpace(c.Query("sc_name"))
	snapshot, err := h.mediaSvc.ReadScMediaMovePlanSnapshot(c.Request.Context(), mode, scName)
	if err != nil {
...
func (h *ScMediaTriggerAPI) Precheck(c *gin.Context) {
	...
	if req.Mode == "" {
		req.Mode = strings.TrimSpace(c.Query("mode"))
	}
	req.Mode = normalizeScMediaMoveReqMode(req.Mode)
	if req.ScName == "" {
		req.ScName = strings.TrimSpace(c.Query("sc_name"))
	}
	if req.Mode == "single" && req.ScName == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "sc_name 为空"})
		return
	}

	result, err := h.mediaSvc.ScMediaMovePrecheck(c.Request.Context(), req.Mode, req.ScName)
...
func (h *ScMediaTriggerAPI) Commit(c *gin.Context) {
	...
	result, err := h.mediaSvc.ScMediaMoveCommit(c.Request.Context(), req.Mode, req.ScName)
...
func (h *ScMediaTriggerAPI) Return(c *gin.Context) {
	...
	if err := h.mediaSvc.ClearScMediaMovePlanSnapshot(c.Request.Context(), req.Mode, req.ScName); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func buildScMediaMovePlanResponse(snapshot *media.ScMediaMovePlanSnapshot) scMediaMoveResponse {
	...
	resp.Mode = normalizeScMediaMoveReqMode(snapshot.Mode)
	...
	switch {
	case resp.Mode == "single" && resp.ScName == "":
...
func buildScMediaMovePrecheckResponse(phase string, result *media.ScMediaMoveResult) scMediaMoveResponse {
	...
	resp.Mode = normalizeScMediaMoveReqMode(result.Mode)
...
func normalizeScMediaMoveReqMode(mode string) string {
